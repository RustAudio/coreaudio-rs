//! A basic output stream example, generating a sine wave tone for tvOS.
//!
//! tvOS does not support microphone input, so this example generates audio output only.
...
pub fn run_example() -> Result<(), coreaudio::Error> {
    ...
    // tvOS doesn't let you reconfigure an "initialized" audio unit, so uninitialize it
